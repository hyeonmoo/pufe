...
//	로그인 버튼
	@PostMapping()
	public String login(String email, String pwd, boolean remEmail, HttpServletRequest req, HttpServletResponse resp,
		...
		if (remEmail)
			resp.addCookie(new Cookie("email", email)); // 이메일 저장버튼 활성화 시 쿠키에 이메일 저장
		else {
...
//	이메일 찾기 버튼
	...
	public String findEmail(String name, String phone, RedirectAttributes ras) throws Exception {
		...
		if (email == null)
			ras.addFlashAttribute("email", "해당 정보로 가입된 계정이 없습니다.");
		else
...
//	비밀번호 재설정-이메일찾기
	...
	public String resetPw_findEmail(String email, String name, String phone, Model m, RedirectAttributes ras)
			throws Exception {
